...
- USB 抜去時は `in_waiting` (ioctl) が素の `OSError` を投げるため `SerialException` と併せて捕捉する
...
- ステータスバー右端に受信バイト数 / レート (1秒ごと更新)
- `QSettings` でポート・ボーレート・更新周期・改行コード・ウィンドウ位置・プロトコル設定 (`ProtocolConfig.to_dict()` を JSON 化) を保存/復元
...
| v1.1.6 | 解析: X軸を0以上に制限、データクリアボタン追加、再接続時のギザギザバグ修正 |
| v1.1.7 | ラベルモード: `key=value` 形式 (printf スタイル) に対応 |
| v1.1.8 | グラフ描画を大幅最適化: データ量が多くても軽量動作 |
| v1.2.0 | 分割受信でバイナリ/CSV(ヘッダ付き)パケットが欠落するバグ修正、USB 抜去時の再接続修正、新ラベル出現時にグラフが消えるバグ修正、CSV エクスポートの列ずれ修正、受信レート表示、送信改行コード選択、設定の保存 |
